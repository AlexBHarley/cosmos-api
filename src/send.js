...
const fetchImplementation =
  typeof fetch !== "undefined" ? fetch : require("isomorphic-fetch");
...
export default async function send(
  ...
  console.log(body);
  const res = await fetchImplementation(`${cosmosRESTURL}/txs`, {
    method: `POST`,
    body
  })
    .then(res => res.json())
...
export async function queryTxInclusion(
  ...
  while (iterations-- > 0) {
    try {
      includedTx = await fetchImplementation(
        `${cosmosRESTURL}/txs/${txHash}`
